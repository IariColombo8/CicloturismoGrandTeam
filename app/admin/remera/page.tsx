...
export default function AdminRemeraPage() {
  ...
  const fetchAlias = async () => {
    const { data } = await supabase
      ...
      .eq("id", "remera")
      .maybeSingle()
    setAliasInfo((data?.data as Record<string, unknown>)?.aliasInfo as string ?? "")
  }
  ...
  const toggleEstado = async (pedido: Remera) => {
    ...
    toast({ title: nuevoEstado === "entregado" ? "Marcado como entregado" : "Revertido a pendiente" })
  }
  ...
  const guardarAlias = async () => {
    ...
    const { error } = await supabase
      .from("content_settings")
      .upsert({ id: "remera", data: { ...dataActual, aliasInfo }, updated_at: new Date().toISOString() })
  ...
  const pedidosFiltrados = useMemo(() => {
    return pedidos.filter((p) => {
      ...
      if (filtroTalle !== "todos" && !p.items.some((i) => i.talle === filtroTalle)) return false
  ...
  const hayFiltros =
    busqueda || filtroEstado !== "todos" || filtroTalle !== "todos" || filtroEnvio !== "todos"
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-800 px-3 py-4 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-yellow-400">Remeras</h1>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatCard label="Total pedidos" value={pedidos.length} icon={<Package className="w-5 h-5" />} />
          <StatCard label="Pendientes" value={pedidos.filter((p) => p.estado === "pendiente").length} icon={<Clock className="w-5 h-5" />} color="text-yellow-400" />
          <StatCard label="Entregados" value={pedidos.filter((p) => p.estado === "entregado").length} icon={<CheckCircle className="w-5 h-5" />} color="text-green-400" />
          <StatCard label="Con envío" value={pedidos.filter((p) => p.envio_tipo === "envio").length} icon={<MapPin className="w-5 h-5" />} />
        </div>
        ...
        {Object.keys(resumenPorTalle).length > 0 && (
          <Card className="bg-zinc-900/50 border-yellow-400/20">
            <CardHeader className="pb-3">
              <CardTitle className="text-yellow-400 text-base">Resumen por talle</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {TALLES_DISPONIBLES.filter((t) => resumenPorTalle[t]).map((talle) => (
                  <div key={talle} className="px-3 py-1.5 bg-zinc-800 rounded-lg border border-zinc-700 text-center">
                    <span className="text-white font-semibold text-sm">{talle}</span>
                    <span className="text-yellow-400 text-sm ml-2">×{resumenPorTalle[talle]}</span>
                  </div>
                ))}
              </div>
            </CardContent>
                  ...
                  {pedidosFiltrados.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-4 py-10 text-center text-zinc-500">
                        No hay pedidos{hayFiltros ? " que coincidan con los filtros" : ""}
                      </td>
                    </tr>
                  ) : (
                    pedidosFiltrados.map((pedido) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          <p className="text-white font-medium">{pedido.nombre}</p>
                          <p className="text-zinc-500 text-xs">{pedido.dni}</p>
                        </td>
                        <td className="px-4 py-3 text-zinc-300">{pedido.telefono ?? "—"}</td>
                        <td className="px-4 py-3 text-zinc-300">{formatItems(pedido.items)}</td>
                        <td className="px-4 py-3">
                            ...
                            {pedido.envio_tipo === "envio" ? (
                              <>
                                ...
                                <span className="text-xs">
                                  Envío
                                  {pedido.direccion && (
                                    <span className="text-zinc-500 block">{pedido.direccion}</span>
                                  )}
                                </span>
                              </>
                            ) : (
                        ...
                        <td className="px-4 py-3">
                          {pedido.esta_registrado ? (
                            <Badge className="bg-green-500/10 text-green-400 border-green-500/20 text-xs">Sí</Badge>
                          ) : (
                            <Badge variant="outline" className="text-zinc-500 border-zinc-700 text-xs">No</Badge>
                          )}
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            >
                              {pedido.estado === "pendiente" ? (
                                ...
                                <><Clock className="w-3.5 h-3.5 mr-1" />Revertir</>
                              )}
                            </Button>
                          </div>
                        </td>
      ...
      <Dialog open={comprobanteOpen} onOpenChange={setComprobanteOpen}>
        <DialogContent className="bg-zinc-900 border-yellow-400/20 max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-yellow-400">Comprobante de pago</DialogTitle>
          </DialogHeader>
          {comprobanteUrl && (
            <div className="space-y-3">
              {comprobanteUrl.endsWith(".pdf") ? (
                <iframe src={comprobanteUrl} className="w-full h-96 rounded border border-zinc-700" />
              ) : (
                <img
                  src={comprobanteUrl}
                  alt="Comprobante"
                  className="w-full max-h-96 object-contain rounded border border-zinc-700"
                />
              )}
              <a
                href={comprobanteUrl}
      ...
      <Dialog open={aliasOpen} onOpenChange={setAliasOpen}>
        <DialogContent className="bg-zinc-900 border-yellow-400/20 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-yellow-400">Datos de pago para remeras</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-zinc-400">
              Este texto se muestra en el formulario público de pedido de remera.
            </p>
            <Textarea
              value={aliasInfo}
            ...
            >
              {savingAlias ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
              Guardar
            </Button>
          </div>
        </DialogContent>
  ...
  return (
    <Card className="bg-zinc-900/50 border-zinc-800">
      <CardContent className="p-4 flex items-center gap-3">
        <div className={`text-yellow-400 ${color === "text-white" ? "" : color}`}>{icon}</div>
        <div>
...
// Importación faltante (usada en el botón de alias)
